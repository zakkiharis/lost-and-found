...
class Temuan extends CI_Controller
{
    ...
    {   
        ...
        if ($id === null) {
            $temuan = $this->temuan->getTemuan();
        }else{
            $temuan = $this->temuan->getTemuan($id);
        }
        

        if ($temuan) {
            $this->response($temuan
            , 200);
        }else{
            $this->response([
                'status' => false,
    ...
    public function index_post()
    {
        $data = [
            ...
            'tempat_temuan' => $this->post('tempat_temuan'),
            'waktu_temuan' => $this->post('waktu_temuan'),
            'nomor_hp' => $this->post('nomor_hp'),
            'id_line' => $this->post('id_line'),
        ];

        if ($this->temuan->createTemuan($data) > 0) {
            $this->response([
                'status' => true,
                'message' => 'data temuan telah ditambahkan'
            ], 201);
        } else {
            $this->response([
                'status' => false,
                'message' => 'Gagal menambahkan data temuan'
            ], 400);
        }
    }
